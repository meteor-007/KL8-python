...
class AutonomousLearner:
    ...
    def autonomous_review(self, period: str = None) -> Dict[str, Any]:
        ...
        learning_state = self.store.get('learning')
    ...
    def self_adjust(self, learnings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        V5.0.2: Level 2允许保守学习(5%幅度)，不再完全跳过
        """
        print(">>> [自主学习] ⚙️ 启动自我调整...")
        level = learnings.get('level', 3)
        adjustments_made = {}

        # V5.0.2: Level 2允许保守学习(5%幅度)，Level 3才完全跳过
        if level >= 3:
        ...
        # 归一化权重 (V5.8.3修复: 原 `'weights' in adjustments_made` 恒为 False —
        # adjustments_made 的键是 "weights.<key>" 前缀格式，从未整体命中死分支。
        # 改为检查是否存在 "weights." 前缀键后再归一化)
    ...
    def self_optimize(self) -> Dict[str, Any]:
        ...
        V5.0.2: consecutive_low阈值从5→3，更快响应低谷期
        新增: 金胆命中率<20%连续3期触发紧急重训
        """
        print(">>> [自主学习] 🧬 启动自我优化...")
        learning_state = self.store.get('learning')
        ...
        # V5.8.3清理: 移除 Tanh 阻尼优化 — 引擎 V5.5 已正式移除 Tanh 阻尼器，
        # tanh_boost_* 参数无任何生产消费者（死参数），此优化项不再写入。
        # V5.0.2新增: 检查金胆命中率是否低于20%连续3期（紧急重训）
        best_hit_rate = learning_state.get('best_golden_hit_rate', 0.0)
        if consecutive_low >= 3 and best_hit_rate < 0.20:
            # 紧急重训: 重置部分权重到默认值
            # V5.8.3: 与 scoring_config.DEFAULT_FEATURE_WEIGHTS 统一（图论共现信号已合并 graph_coupling=0.431）
            default_weights = {
...
def batch_update(history_results, min_batch=50):
    ...
    if len(history_results) < min_batch:
        return False
    # 占位：真实重拟合逻辑在后续任务接入（hit/score 抽取后调模型重训）
    # 此处先保证门控正确：样本足够才返回 True
    return True
