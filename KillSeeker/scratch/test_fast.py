...
def run_backtest_with_weights(data_loader, config, weights, n_periods=10):
    ...
    for i in range(n_periods):
        ...
        try:
            ...
            print(f"Error at period {target_period}: {e}", flush=True)
            continue
...
def main():
    ...
    print("\n--- Running 10-period Backtests ---", flush=True)
    for name, weights in test_cases:
        kr, hr = run_backtest_with_weights(data_loader, config, weights, n_periods=10)
        print(f"{name:<45} | Total Kill Hit: {kr:.2%} | High Conf Hit: {hr:.2%}", flush=True)
