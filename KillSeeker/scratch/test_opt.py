...
def main():
    ...
    test_cases = [
        ...
        ("Option A (Curve heavy)", {
            "similarity": 0.30,
            "density": 0.30,
            "pattern": 0.0,
            "curve": 0.40,
        }),
        ("Option B (Similarity heavy)", {
            "similarity": 0.40,
            "density": 0.20,
            "pattern": 0.0,
            "curve": 0.40,
        }),
        ("Option C (Density light)", {
            "similarity": 0.35,
            "density": 0.15,
            "pattern": 0.0,
            "curve": 0.50,
        }),
        ("Option D (Similarity light)", {
            "similarity": 0.20,
            "density": 0.40,
            "pattern": 0.0,
            "curve": 0.40,
    ...
    print("\n--- Running 30-period Fine-tuned Backtests ---", flush=True)
    for name, weights in test_cases:
        kr, hr, mr = run_backtest_with_weights(data_loader, config, weights, n_periods=30)
        print(f"{name:<45} | Total Kill Hit: {kr:.2%} | High Conf Hit: {hr:.2%} | Mid Conf Hit: {mr:.2%}", flush=True)
